...
class ProfileVelocityMode : public OperationModeInterface {
public:
  ProfileVelocityMode(const std::shared_ptr<EposActuatorContext> &context)
      : OperationModeInterface("profile_velocity", context) {}

  virtual void starting() override {
    try {
      // switch to profile velocity mode
      *context_->node.set_enable_state();
      *context_->node.activate_profile_velocity_mode();
  ...
  virtual void write(const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/) override {
    ...
    try {
      if (!std::isnan(context_->vel_cmd) && context_->vel_cmd != prev_vel_cmd_) {
        *context_->node.move_with_velocity(context_->vel_cmd);
